...
export function makeFullFilter(filter: ExportsFilter): ExportsFilter {
    if (Reflect.has(filter, fullFilterSym)) {
        throw Error(`Attempted to double wrap a full filter which can lead to undefined webpack crawling behaviour when dealing with circular dependencies.`);
    }
...
export const byPropsFilter = (...props: string[]): ExportsFilter => (exports) =>
    isSearchableObj(exports) && props.every(prop => Reflect.has(exports, prop));

export const byFactoryCode = (match: string | RegExp): FactoryFilter => (factory) => {
...
export const byStoreName = (store: string): ExportsFilter => (exports) =>
